...
export function QuickAddDialog({
  ...
  // Reset selections every time the dialog opens, seeding to the first
  // in-stock option so users never see a pre-selected OOS variant.
  useEffect(() => {
    if (open) {
      setVariantSize(firstAvailableVariant);
      setFlavour(firstAvailableFlavour);
      setQty(minQty);
    }
  }, [open, product.id, firstAvailableVariant, firstAvailableFlavour, minQty]);
